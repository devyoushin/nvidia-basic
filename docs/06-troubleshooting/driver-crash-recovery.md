...
# 드라이버 버전 파일 존재 여부
cat /proc/01-driver/nvidia/version 2>&1
```
...
# docs/06-troubleshooting/aws-host-replacement.md 참조
```
...
- 드라이버 크래시 발생 시 로그 자동 보존 스크립트를 cron 또는 systemd path unit으로 구성: `/proc/01-driver/nvidia/version` 파일 사라지면 자동 dmesg 저장
...
**관련 문서**
- 연관 내부 문서: `docs/06-troubleshooting/xid-error-codes.md`, `docs/06-troubleshooting/aws-host-replacement.md`, `docs/01-driver/nvidia-driver-install-al2023.md`
